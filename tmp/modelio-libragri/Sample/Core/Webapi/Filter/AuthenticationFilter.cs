...
namespace Core.Webapi.Filter
{
    ...
    /// {
    ///     ...
    ///     services.AddMvc(op => op.Filters.Add<AuthenticationFilter>());
    /// }
    /// </summary>
    public class AuthenticationFilter : IAsyncActionFilter
    {
        ...
        public AuthenticationFilter(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            _httpContextAccessor = httpContextAccessor;
            _AuthHeaderKey = configuration["Auth0:AuthorizationHeaderKey"];
        ...
        private static void AuthenticationCheck(IHeaderDictionary headers)
        {
            ...
            var headerPair = headers.FirstOrDefault(o => o.Key == _AuthHeaderKey);

            if (headerPair.Key == null)
                throw new AuthenticationException($"No  Authorization header was present. Expecting {_AuthHeaderKey}: Bearer xxxxxx");

            var header = headerPair.Value.FirstOrDefault();

            if (header == null)
                throw new AuthenticationException($"No  Authorization header was present. Expecting this header key : {_AuthHeaderKey}");

            if (!AuthenticationHeaderValue.TryParse(header, out var authorization)
                || authorization == null
                || (!AllowedBearerString.Contains(authorization.Scheme)))
                //todo
                throw new AuthenticationException($"The  Authorization header is in the wrong format. Expecting Authzorization: Bearer {{JWT}}");
        ...
        public static string GetRessourceName()
        {
            switch (_environment)
            {
                case "dev": return "my_dev.cer";

                case "validation": return "my_validation.cer";

                case "preprod": return "my_preprod.cer";

                case "prod": return "my_prod.cer";

                default: return "my_dev.cer";
            }
