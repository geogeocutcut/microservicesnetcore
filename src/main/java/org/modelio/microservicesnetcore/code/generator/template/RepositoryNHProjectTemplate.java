...
public class RepositoryNHProjectTemplate implements InterfaceRepoProjectTemplate {
	private String _header= "org/modelio/microservicesnetcore/template/02 - repository/impl/nhibernate/header.txt";
	...
	private String _csproj = "org/modelio/microservicesnetcore/template/02 - repository/impl/nhibernate/csproj.txt";
	private String _unitofwork = "org/modelio/microservicesnetcore/template/02 - repository/impl/nhibernate/unitofwork.txt";
	private String _iocregister = "org/modelio/microservicesnetcore/template/02 - repository/impl/nhibernate/iocregister.txt";
	private String _embeddedresource = "org/modelio/microservicesnetcore/template/02 - repository/impl/nhibernate/embeddedresource.txt";
	
	private String _applicationName;
	private Package _domain;
	
	public RepositoryNHProjectTemplate(String applicationName, Package domain) {
		_domain=domain;
	...
	public String getEmbedded(List<String> mappingFiles) {
		// TODO Auto-generated method stub
		StringBuilder tmpl = new StringBuilder();
		try {
			InputStream stream = getClass().getClassLoader().getResourceAsStream(_embeddedresource);
			BufferedReader  reader = new BufferedReader (new InputStreamReader(stream, Charset.forName("UTF-8")));
			while (reader.ready()) {
				tmpl.append(reader.readLine()).append("\n");
			}
		...
		{
			result += tmpl.toString().replaceAll("@@file", "Config\\\\"+mappingFile);
		}
